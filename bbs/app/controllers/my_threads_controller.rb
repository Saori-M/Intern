class MyThreadsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @my_thread.update(my_thread_params)
        format.html { redirect_to my_thread_path(@my_thread.id), notice: 'My thread was successfully updated.' }
  ...
  def destroy
    ...
    @my_thread.destroy
    respond_to do |format|
      format.html { redirect_to my_threads_path notice: 'My thread was successfully destroyed.' }
  ...
  private
    ...
    def my_thread_params
      params.require(:my_thread).permit(:title, :content)
    end
end
